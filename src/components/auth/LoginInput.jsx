...
const LoginInput = ({login}) => {
  ...
  return (
    <form onSubmit={onSubmitHandler} className='flex flex-col gap-5 w-full'>
      ...
      <input type='password' placeholder='Password' value={password} onChange={onPasswordChangeHandler} className='w-full border-2 p-1 note-create__title-input border-[#aaa]' />
      <button className='w-full border-2 p-2 bg-primary cursor-pointer' type='submit'>Login</button>
    </form>
  );
}
